...
class CompatibilityCalculator:
    ...
    ENHANCED MODEL v3 - Score = weighted sum of 12 factors:
    1. Mentorship Potential (20%)
    2. Network Value (16%)
    3. Skill Learning (16%)
    4. Role Synergy (12%)
    5. Goal Alignment (5%) - NEW!
    6. Alumni Connection (9%)
    7. Career Advancement (9%)
    8. Industry Match (7%)
    9. Geographic Proximity (3%) - NEW!
    10. Engagement Quality (2%) - NEW!
    11. Rising Star Potential (1%) - NEW!
    12. Red Flag Penalty (multiplier) - NEW!
    """
    
    # Weight distribution (ENHANCED v3)
    WEIGHTS = {
        'mentorship_potential': 0.20,
        'network_value': 0.16,
        'skill_learning': 0.16,
        'role_synergy': 0.12,
        'goal_alignment': 0.05,      # NEW!
        'alumni_connection': 0.09,
        'career_advancement': 0.09,
        'industry_match': 0.07,
        'geographic_proximity': 0.03,  # NEW!
        'engagement_quality': 0.02,    # NEW!
        'rising_star': 0.01           # NEW!
        # red_flag_penalty applied as multiplier
    }
    ...
    def calculate(self, user_profile, target_profile):
        """Calculate compatibility score (0-100) - ENHANCED v3"""
        ...
        # 5. Goal Alignment (5%) - NEW v3!
        scores['goal_alignment'] = self._goal_alignment_score(user_profile, target_profile)
        ...
        # 9. Geographic Proximity (3%) - NEW v3!
        scores['geographic_proximity'] = self._geographic_proximity_score(user_profile, target_profile)
        
        # 10. Engagement Quality (2%) - NEW v3!
        scores['engagement_quality'] = self._engagement_quality_score(target_profile)
        
        # 11. Rising Star (1%) - NEW v3!
        scores['rising_star'] = self._rising_star_score(target_profile)
        
        # Calculate weighted total
        total = sum(self.WEIGHTS[k] * scores[k] for k in scores)
        
        # 12. Apply Red Flag Penalty (multiplier) - NEW v3!
        red_flag_multiplier = self._red_flag_penalty(target_profile)
        total = total * red_flag_multiplier
        
        return {
            'total_score': round(total, 2),
            'breakdown': scores,
            'red_flag_multiplier': red_flag_multiplier
        }
    ...
    def _goal_alignment_score(self, user, target):
        """Do target's offerings match user's needs? (NEW v3)"""
        user_needs = user.get('needs', [])
        target_offers = target.get('can_offer', [])
        
        if not user_needs or not target_offers:
        ...
        if isinstance(user_needs, str):
            import ast
            try:
                user_needs = ast.literal_eval(user_needs)
            except:
                user_needs = []
        
        if isinstance(target_offers, str):
            import ast
            try:
                target_offers = ast.literal_eval(target_offers)
            except:
                target_offers = []
        
        # Calculate overlap
        user_needs_set = set([str(n).lower() for n in user_needs])
        target_offers_set = set([str(o).lower() for o in target_offers])
        
        overlap = user_needs_set & target_offers_set
        
        if not user_needs_set:
            return 50
        
        match_rate = len(overlap) / len(user_needs_set)
        
        # Score based on match rate
        score = int(match_rate * 100)
        return min(max(score, 20), 100)  # Clamp between 20-100
    
    def _geographic_proximity_score(self, user, target):
        """Are they in the same location? (NEW v3)"""
        user_loc = str(user.get('location', '')).strip()
        target_loc = str(target.get('location', '')).strip()
        
        if not user_loc or not target_loc:
            return 50  # Neutral if no location data
        
        # Parse location (typically "City, State")
        try:
            user_parts = user_loc.split(',')
            target_parts = target_loc.split(',')
            
            user_city = user_parts[0].strip().lower() if len(user_parts) > 0 else ""
            user_state = user_parts[1].strip().lower() if len(user_parts) > 1 else ""
            target_city = target_parts[0].strip().lower() if len(target_parts) > 0 else ""
            target_state = target_parts[1].strip().lower() if len(target_parts) > 1 else ""
            
            # Same city
            if user_city and target_city and user_city == target_city:
                score = 100
            # Same state
            elif user_state and target_state and user_state == target_state:
                score = 70
            # Different locations
            else:
                score = 40
            
            # Bonus for remote preference
            user_remote = str(user.get('remote_preference', '')).lower()
            target_remote = str(target.get('remote_preference', '')).lower()
            
            if 'remote' in user_remote and 'remote' in target_remote:
                score = min(score + 10, 100)
    ...
    def _engagement_quality_score(self, target):
        """How active/engaged is target on LinkedIn? (NEW v3)"""
        engagement = target.get('engagement_quality_score', 50.0)
        
        # engagement_quality_score is already 0-100
        return float(engagement)
    
    def _rising_star_score(self, target):
        """Is target an up-and-comer? (NEW v3)"""
        rising_star = float(target.get('rising_star_score', 0.0))
        hidden_gem = float(target.get('undervalued_score', 0.0))
        
        # Average of both scores
        score = (rising_star + hidden_gem) / 2
        return score
    
    def _red_flag_penalty(self, target):
        """Calculate penalty multiplier for red flags (NEW v3)"""
        red_flag_score = float(target.get('red_flag_score', 0.0))
        
        # Higher red_flag_score = more problems = lower multiplier
        if red_flag_score > 75:
            return 0.80  # -20% penalty
        elif red_flag_score > 50:
            return 0.90  # -10% penalty
        elif red_flag_score > 25:
...
for i in range(0, len(profile_indices), 2):
    ...
    pairs_data.append({
        ...
        'goal_alignment_score': result['breakdown']['goal_alignment'],  # NEW v3
        ...
        'geographic_score': result['breakdown']['geographic_proximity'],  # NEW v3
        'engagement_score': result['breakdown']['engagement_quality'],  # NEW v3
        'rising_star_score_calc': result['breakdown']['rising_star'],  # NEW v3
        'red_flag_multiplier': result.get('red_flag_multiplier', 1.0),  # NEW v3
...
🚀 Next Step: Train the model (run 3_train_model.py with v3 data)
""".format(len(pairs_df), 
             pairs_df['compatibility_score'].min(), 
             pairs_df['compatibility_score'].max(),
             pairs_df['compatibility_score'].mean()))
